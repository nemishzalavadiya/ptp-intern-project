...
export default function Signature({leagleInfo,setLeagleInfo,upload,nextClick,isValid}) {
  ...
  return (
    <div>
      <Loader inverted active={isExecuting}>Verifing</Loader>
      <Segment placeholder className="signature">
      <Segment>
      <Header icon>
      <Icon name='images file outline' />
      {leagleInfo.sign===null?"Upload Signature":leagleInfo.sign.name} 
    </Header>
    ...
    <Header icon>
      ...
      {leagleInfo.profile===null?"Upload Profile Picture":leagleInfo.profile.name} 
    </Header>
    <label className="custom-file-upload">
  ...
  <Button className="kycbutton" onClick={uploadKYCDetails} disabled={isExecuting}>Upload Now</Button>
  </div>
