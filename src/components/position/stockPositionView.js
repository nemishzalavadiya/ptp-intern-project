...
const stockHeaders = [
  ...
  { header: "Average Price", icon: <i className="rupee sign icon small"></i> },
  { header: "Cost", icon: <i className="rupee sign icon small"></i> },
  { header: "Current Price", icon: <i className="rupee sign icon small"></i> },
...
export default function StockPosition({uuid,positionList,pagination})
{
  ...
  if(isSubscriptionCompleted)
  {
        Array.from(myMap.values()).forEach((row,index) => {
          ...
          positionList[index][6]=((netValue-positionList[index][3])/(positionList[index][3])*100);
  ...
  <div>
    {
       !isSubscriptionCompleted?<Loader active/>:null
    }
  </div>
   <GridContainer content={isSubscriptionCompleted?stockHeaders:stockHeaders.slice(0,4)} pagination={pagination} data={positionList}></GridContainer> 
  </div>);
} 