...
async function fetchUrl(Url) {
    ...
    await content.forEach(element => {
        ...
        positionList.push([<Link href={`/details/${id}`}>{name}</Link>,volume,price,price/volume,nav,netValue,(profit * price) / 100,profit]);
    });
  ...
  useEffect(() => {
      setStatus(false);
     fetchUrl(`/api/mutualfund/position/search/users/${userId}?name=${searchText}&page=${page}&size=${size}`);
