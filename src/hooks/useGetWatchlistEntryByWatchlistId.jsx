...
const useWatchlist = (watchlistId, searchQuery, page, size, option) => {
  ...
  async function getAllWatchlistEntryByWatchlistId() {
    ...
    if (response.ok) {
      ...
      setContent({
        data: { companyUuids: companyUuids, totalPages: data.totalPages },
        error: false,
        isComplete: true,
      });
    } else {
  ...
  }, [watchlistId, searchQuery, page, size, option]);
