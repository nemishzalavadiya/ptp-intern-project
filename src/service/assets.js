...
function getAllAssets(){
    const [isComplete,data] = useFetch('/assets?page=0&size=10');
    return [isComplete,data];
}
function getAssetById(id){
    const [isComplete,data] = useFetch('/assets/'+id);
    return [isComplete,data];
}
function getStockByAssetId(id){
    const [isStockFetched, stockDetail] = useFetch('/assets/stocks/'+id);
    console.log("stock : ", stockDetail);
    return [isStockFetched, stockDetail];
}
function getMfByAssetId(id){
    const [isMfFetched, mfDetail] = useFetch('/assets/mutualfunds/'+id);
